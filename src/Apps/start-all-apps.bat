...
REM Set CI environment variable to bypass rollup issue
set CI=true
...
start cmd /k "cd LiquidityProvider\quantum-flow-ef2039a0 && set VITE_PORT=5174 && npm run dev -- --port 5174"

echo.
echo Starting ManagementPortal on port 5175...
start cmd /k "cd ManagementPortal\quantum-sky-link-admin-console-a1e95f2e && set VITE_PORT=5175 && npm run dev -- --port 5175"

echo.
echo Starting TokenPortal on port 5176...
start cmd /k "cd TokenPortal\quantum-mint-dcb09fa1 && set VITE_PORT=5176 && npm run dev -- --port 5176"
